...
export const cortApi = {
	...
	// bosses.php/events.json/stats.json all send either a *fixed*
	// Access-Control-Allow-Origin: https://cort.ovh (never our own origin)
	// or, on bosses.php, no CORS header at all — either way, a browser
	// fetching these directly always has the response withheld by CORS
	// regardless of connection quality. Routed through our own same-origin
	// proxy (api/cort-proxy.ts) instead, which fetches them server-side
	// (not subject to CORS) and relays the JSON back — with no caching
	// anywhere in that path, so this always reflects the current live status.
	bosses: () => getJSON<BossSpawnData>("/api/cort-proxy?endpoint=bosses"),
	// Fort/gem/relic status — scraped from championsofregnum.com itself
	// instead of relaying cort.ovh's wstatus.json (see api/wz-official.ts).
	// No events history in that source, so fort "captured Xh ago" labels
	// won't have anything to show — events.json below still comes from
	// cort.ovh for that reason.
	warzoneStatus: () => getJSON<WzStatusData>("/api/wz-official"),
