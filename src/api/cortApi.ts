...
// Used by the "live" pages (WZ status, BZ, events, stats). Boss respawns
// used to be here too (bosses.php) — CoRT's own v5 rewrite removed that
// endpoint entirely and moved the computation client-side instead (see
// `features/bosses/bossScheduleEngine.ts`), so there's nothing left to
// call for that. These remaining endpoints are not needed by the Trainer
// (which uses bundled/static reference data), but are wired up here so
// those pages are a small step away from being built.
//
...
export const cortApi = {
	...
	// so a browser fetching them directly always has the response withheld
