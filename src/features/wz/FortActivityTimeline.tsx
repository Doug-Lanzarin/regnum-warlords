...
const GRANULARITY_LABEL: Record<Granularity, string> = { 15: "15 min", 30: "30 min", 60: "1h" };
		...
		<section className={styles.section}>
			<div className={styles.header}>
				<div className={styles.titleGroup}>
					<h2>Curva de atividade</h2>
					<span className={styles.count}>{total} fortes nas últimas 24h, por horário</span>
				</div>
			...
			<div className={`card ${styles.card}`}>
				<div className={styles.chartWrap}>
					<svg
						...
						role="img"
						aria-label={`Curva de fortes capturados nas últimas 24h, ${total} no total`}
					...
					>
						...
						{yTicks.map((t) => (
							<text key={t} x={PAD.left - 8} y={yAt(t)} className={styles.yLabel} textAnchor="end" dominantBaseline="middle">
								{t}
							</text>
						))}
						...
						{tickIndices.map((i, tickPos) => {
							...
							return (
								<text key={i} x={xAt(i)} y={VB_H - 6} className={styles.xLabel} textAnchor={anchor}>
									{formatHourMinute(buckets[i]?.time ?? now)}
								</text>
							);
						})}
					...
					{hovered && hoverIndex !== null && (
						...
						>
							<strong>{hovered.count}</strong> {hovered.count === 1 ? "forte" : "fortes"}
							<span className={styles.tooltipTime}>{formatHourMinute(hovered.time)}</span>
						</div>
					)}
				</div>
			</div>
